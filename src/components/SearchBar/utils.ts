...
export const fetchAddr = async ({
  ...
  const addr = await Resolver.default.resolveDns(
    network,
    initTyron,
    username,
    domain
...
export const resolve = async ({
  ...
  await DidDocument.default.resolution(network, resolution_input)
  .then(async did_resolved => {
      ...
      if(doc.authentication !== undefined) {
        did_doc.push(['Authentication public key: ', [doc.authentication.publicKeyBase58]]);
      }
      if(doc.assertionMethod !== undefined) {
        did_doc.push(['Assertion public key: ', [doc.assertionMethod.publicKeyBase58]]);
      }
      if(doc.capabilityDelegation !== undefined) {
        did_doc.push(['Delegation public key: ', [doc.capabilityDelegation.publicKeyBase58]]);
      }
      if(doc.capabilityInvocation !== undefined) {
        did_doc.push(['Invocation public key: ', [doc.capabilityInvocation.publicKeyBase58]]);
      }
