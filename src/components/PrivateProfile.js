                  ...
                  try {
                    ...
                    if (arconnect !== '') {
                      ...
                      switch (keyId) {
                        case 'ssiComm':
                          {
                            const ssiCommKeys = await DKMS.generateSsiKeys(
                              arweave
                            );
                            const ssiCommPrivate = await DKMS.encryptKey(
                              arconnect,
                        ...
                        case 'byId':
                          {
                            ...
                            const key = await DKMS.encryptKey(
                              arconnect,
                              keys.privateKey
                            );
                            input = {
                      ...
                      switch (keyId) {
                        case 'ssiComm':
                          {
                            ...
                            const ssiCommPrivate = await DKMS.encryptData(
                              ssiCommKeys.privateKey,
                              publicEncryption
                            );
                            input = {
                        ...
                        case 'byId':
                          {
                            const keys = await DKMS.generateSsiKeys(arweave);
                            const key = await DKMS.encryptData(
                              keys.privateKey,
                              publicEncryption
                            );
                            input = {
